...
export function emitNetPromise<T>(eventName: string, ...args: any[]): Promise<PromiseEventResponse<T>> {
  return new Promise((res, rej) => {
    ...
    const handleRespEvent = (serverResp: PromiseEventResponse<T>) => {
      ...
      if (timedOut) return;
      if (serverResp.error) rej(`Error for response event ${eventName}: ${serverResp.errorMsg}`)
      res(serverResp)
    }
